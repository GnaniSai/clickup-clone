...
const Toggle = ({ onClick, isEnabled }: { onClick: () => void, isEnabled: boolean }) => {
    return (
      <button
        onClick={onClick}
        className={`w-14 h-7 rounded-full p-1 flex items-center cursor-pointer transition-all duration-300 ${
          isEnabled ? "bg-[#7612fa]" : "bg-[#9e9ebb]"
        }`}
      >
        <div
  ...
  export const Flexibility = () => {
            ...
            {customizeClickData.map((item, index) => {
              ...
              return (
                <div className="px-2 pb-1" key={index}>
                  <div
                    className={`flex items-center justify-between py-4 px-7 ${
                      isEnabled ? "border border-[#6060a34d] rounded-xl shadow-lg bg-white" : "border border-transparent"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-md font-medium ${
                          isEnabled ? "text-[#5037E7]" : "text-[#5d5d85]"
                        }`}
                      >
                        {item.title}
                      </span>
    ...
    return (
        <div className="flex flex-col items-center justify-center mx-50 max-xl:mx-20 my-20 max-md:hidden">
            ...
            <div className="grid grid-cols-2 grid-rows-2 gap-7 mx-50 max-xl:mx-20 my-10">
